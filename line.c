 ...
 * Return: bytes read
 */
ssize_t input_buffer(info_t *parg, char **buf, size_t *len)
{
ssize_t r = 0;
size_t len_p = 0;
...
remove_comments(*buf);
build_history_list(parg, *buf, parg->histcount++);
 ...
 * Return: s
 */
int _getnxtline(info_t *parg, char **ptp, size_t *size)
{
...
check_chain(parg, buf, &j, i, len);
while (j < len) /* iterate to semicolon or end */
{
if (is_chain(parg, buf, &j))
break;
j++;
}
 ...
 * Return: r
 */
ssize_t read_buffer(info_t *parg, char *buf, size_t *s)
{
ssize_t r = 0;
if (*i)
return (0);
