...
{
ssize_t fd;
char *filename = get_thehistory_file(info);
list_t *node = NULL;
if (!filename)
return (-1);
...
/**
 * read_file_history - reads history from file
 * @info: the parameter struct
...
struct stat st;
char *buf = NULL, *filename = get_thehistory_file(info);
if (!filename)
return (0);
...
/**
 * renumber_history_link - renumbers the history linked list after changes
 * @info: Structure containing potential arguments. Used to maintain
 * Return: the new histcount
 */
int renumber_history_link(info_t *info)
{
